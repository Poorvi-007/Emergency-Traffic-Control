...
def process_frame(frame):
    ...
    for result in emergency_results:
        ...
        for box in result.boxes:
            ...
            vehicle = emergency_model.names[class_id]
    ...
    # =================================================
    # UPDATE EMERGENCY + REAL-TIME TRACKING
    # =================================================
    ...
    if emergency_found:
        ...
        if last_emergency_center is not None:

            previous_x, previous_y = last_emergency_center

            dx = center_x - previous_x
            dy = center_y - previous_y

            movement_pixels = (dx * dx + dy * dy) ** 0.5

            if movement_pixels < 5:
                movement = "STATIONARY"
            elif dy > 5:
        ...
        if (
            missed_emergency_frames
            > MAX_MISSED_EMERGENCY_FRAMES
        ):
    ...
    for junction in [
        ...
        zones[junction] = {
            ...
            "density":
                calculate_density(count),

            "congestion":
                calculate_congestion(count)
...
def generate_video():
    ...
    while True:
        ...
        if detection["detected"]:
            ...
            track_text = (
                f'Tracking: {detection["track_id"]} | '
                f'Movement: {detection["movement"]}'
            )
            ...
            # Draw tracking trail
            points = detection.get("track_points", [])
